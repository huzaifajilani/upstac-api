...
public class LabRequestController {
    ...
    public List<TestRequest> getForTester()  {
        ...
        //Create new a new ArrayList
        List<TestRequest> testRequestList = new ArrayList<TestRequest>();

        try {
            currentUser = userLoggedInService.getLoggedInUser();
            if (currentUser.doesRoleIsTester())
                testRequestList = testRequestQueryService.findByTester(currentUser);
        }
        catch (Exception exp){
            throw asBadRequest(exp.getMessage());
        }
    ...
    public TestRequest updateLabTest(@PathVariable Long id,@RequestBody CreateLabResult createLabResult) {
        ...
        catch (ConstraintViolationException consViolationExp) {
            throw asConstraintViolation(consViolationExp);
        }
        catch (AppException appExp) {
            throw asBadRequest(appExp.getMessage());
        }
    }
